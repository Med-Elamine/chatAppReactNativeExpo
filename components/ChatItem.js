...
const RenderChatItem = ({props}) => {
    ...
    let avatarImage = item.image ?? unknownAvatarImage;
        
        return(
            <View
                style={[
                    Styles.flatListItem,
                    { borderColor: props.username == props.item.by ? "green" : "blue" },
                ]}
            >
            <View style={Styles.chatItemHeader}>
              ...
              <Text style={Styles.smallItalicText}>
                {props.item.by} at {new Date(props.item.timeStamp).toLocaleTimeString()}
              </Text>
            </View>
            <Text style={Styles.chatText}>{props.item.text}</Text>
          </View>);
}
